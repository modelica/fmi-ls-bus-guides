...
fmi3Status fmi3GetBinary(fmi3Instance instance, 
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == TX_DATA_REFERENCE) {
            values[i] = FMI3_LS_BUS_BUFFER_START(&TxBufferInfoCan);        // <4>
            valueSizes[i] = FMI3_LS_BUS_BUFFER_LENGTH(&TxBufferInfoCan);   // <5>
        }
    }
    ...
}
...
fmi3Status fmi3UpdateDiscreteStates(...) {
    ...
    FMI3_LS_BUS_BUFFER_INFO_RESET(&TxBufferInfoCan);    // <6>
    ...
}
----
...
fmi3Status fmi3SetBinary(fmi3Instance instance,
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == RX_DATA_REFERENCE) {
            /* Reset read/write positions */
            FMI3_LS_BUS_BUFFER_INFO_RESET(&RxBufferInfoCan);                    // <3>    
            /* Write buffer */
            FMI3_LS_BUS_BUFFER_WRITE(&RxBufferInfoCan, value[i], valueSize[i]); // <4>
        }
    }
    ...
}
...
fmi3Status fmi3SetBinary(fmi3Instance instance,
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == RX_DATA_REFERENCE) {
            /* Reset read/write positions */
            FMI3_LS_BUS_BUFFER_INFO_RESET(&RxBufferInfoFlexRay);                    // <3>
            /* Write buffer */    
            FMI3_LS_BUS_BUFFER_WRITE(&RxBufferInfoFlexRay, value[i], valueSize[i]); // <4>
        }
    }
    ...
}
