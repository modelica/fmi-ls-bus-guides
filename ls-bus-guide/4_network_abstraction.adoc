...
image::images/create_process_bus_operations.svg[width=600]
...
Within the layered standard, the https://modelica.github.io/fmi-ls-bus/main/#low-cut-variables[connection] between the data to be exchanged (`TX_Data` and `Rx_Data`) and the time of exchange (`Tx_Clock` and `Rx_Clock`) has been well defined.
The https://modelica.github.io/fmi-ls-bus/main/#low-cut-tx-triggered-clocks[simplest case] represents a `triggered` `Tx_Clock` that basically allows to signal events when returning from `fmi3DoStep`.
...
It should be clear that, depending on the application, the different FMI clock types each offer advantages and disadvantages. See also the https://modelica.github.io/fmi-ls-bus/main/#low-cut-tx-clocks[corresponding chapter in the layered standard].
...
image::images/can_transmission_acknowledge.svg[width=700]
...
image::images/can_arbitration_overview.svg[width=1000]

NOTE: Note: The path shown represents the intended approach.
Other implementations may also be valid according to the FMI specification.
...
image::images/can_arbitration_overview_with_buffer.svg[width=1000]
...
image::images/can_error_state_machine.svg[width=600]
...
This chapter contains details on Ethernet specific topics.

==== Getting Started with Ethernet [[low-cut-ethernet-getting-started-with-ethernet]]

This section shows the exemplary implementation in conjunction with the provided header files of the FMI-LS-BUS layered standard.

===== Provided Header Files [[low-cut-ethernet-getting-started-provided-header-files]]

Besides the textual specification for FMUs with bus support, the Layered Standard for Network Communication also provides a C API to make the creation of FMUs with bus support as easy and generalized as possible.

* https://github.com/modelica/fmi-ls-bus/blob/main/headers/fmi3LsBus.h[fmi3LsBus.h] provides general macros, types and structures of common Bus Operations.
This header file applies to all supported bus types of the layered standard.
* https://github.com/modelica/fmi-ls-bus/blob/main/headers/fmi3LsBusEthernet.h[fmi3LsBusEthernet.h] provides macros, types and structures of Bus Operations explicit for Ethernet.
Primarily, structures are included here that allow the Bus Operations specified by the layered standard to be easily created and used.
* https://github.com/modelica/fmi-ls-bus/blob/main/headers/fmi3LsBusUtil.h[fmi3LsBusUtil.h] provides common utility macros and structures for all supported bus types.
...
This can be done either in `Step Mode` or in `Event Mode`.
For a transfer, all Bus Operations are serialized together in a `fmi3Binary` https://modelica.github.io/fmi-ls-bus/main/#low-cut-tx-rx-data-variables[variable].
This serialization can be done using the provided <<low-cut-ethernet-getting-started-provided-header-files,header files>> or using a custom approach.
After the contents of the binary variables have been transferred to another FMU via an FMU importer, they are deserialized again and then processed inside the receiver.

.Create, process and transfer Bus Operations.
[#figure-general-aspects-overview-ethernet]
image::images/create_process_bus_operations.svg[width=600]
...
Details about possible entry points regarding the FMI interface will be provided later in this section.
However, it is important to know that due to the resulting decoupling between the generation, processing and transfer of Bus Operations, the need for buffer semantics arises.
This semantics is reflected directly in the structure and handling of the <<low-cut-ethernet-getting-started-provided-header-files,header files>> provided.
...
fmi3UInt8 TxBufferEth[2048];    // <2>
fmi3UInt8 RxBufferEth[2048];
fmi3LsBusUtilBufferInfo TxBufferInfoEth;    // <3>
fmi3LsBusUtilBufferInfo RxBufferInfoEth;

fmi3Instance fmi3InstantiateCoSimulation(...) {
    FMI3_LS_BUS_BUFFER_INFO_INIT(&TxBufferInfoEth, TxBufferEth, sizeof(TxBufferEth));    // <4>
    FMI3_LS_BUS_BUFFER_INFO_INIT(&RxBufferInfoEth, RxBufferEth, sizeof(RxBufferEth));
}
----
<1> Necessary include of the fmi3LsBusUtil.h header file.
<2> Definition and initialization of a `fmi3Binary` buffer variable.
...
The header file https://github.com/modelica/fmi-ls-bus/blob/main/headers/fmi3LsBusUtilEthernet.h[fmi3LsBusUtilEthernet.h] offers macros for all Bus Operations specified by the layered standard, which minimize the effort required to create and serialize such an operation.
The macros are always provided according to the following syntax: `FMI3_LS_BUS_<BusType>_CREATE_OP_<OperationName>`.
Following these rule, the macro for creating a Ethernet Transmit operation is `FMI3_LS_BUS_ETHERNET_CREATE_TRANSMIT`.
A macro for creating an operation also assigns it to a buffer described by an `fmi3LsBusUtilBufferInfo` instance.

The following program code shows how to first define the payload and the destination address that should be used in the Ethernet Transmit operation.
Afterwards, the `fmi3LsBusUtilBufferInfo` is reset by using `FMI3_LS_BUS_BUFFER_INFO_RESET`.
`FMI3_LS_BUS_BUFFER_INFO_RESET` sets the internal position of the `fmi3LsBusUtilBufferInfo` instance to zero, so that it is essentially emptied and written from the beginning.
This is necessary to ensure that Bus Operations that have already been transmitted are not transmitted again.
`FMI3_LS_BUS_ETHERNET_CREATE_OP_TRANSMIT` now creates a new Ethernet Transmit operation with the associated parameters such as source address, target address and payload and adds them directly to the `fmi3LsBusUtilBufferInfo` instance.
Querying the status of a `fmi3LsBusUtilBufferInfo` instance allows you to check whether there is still enough space in the underlying buffer.
In the last step, `FMI3_LS_BUS_BUFFER_LENGTH` is used to check whether there are Bus Operations in the respective `fmi3LsBusUtilBufferInfo` variable that should be transmitted in `Event Mode`.

[source,c]
.Creation of a Transmit operation
----
#include "Fmi3LsBusUtilEthernet.h" 

fmi3Status fmi3DoStep(..., eventHandlingNeeded, ...) { 
    fmi3UInt8 msg[] = "Hey guys";                                // <1>
    fmi3LsBusEthernetMacAddressOctet destination[] = "ABCDEF";   // <2>

    /* Reset read/write positions of the BufferInfo variable */
    FMI3_LS_BUS_BUFFER_INFO_RESET(&TxBufferInfoEth);             // <3>

    /* Create a Transmit operation to be send */
    FMI3_LS_BUS_ETHERNET_CREATE_OP_TRANSMIT(&TxBufferInfoEth, ..., destination, sizeof(msg), msg);    // <4>

    if (!TxBufferInfoEth.status) {   // <5>
        /* Error: No free buffer space available */
    }

    ...

    if (FMI3_LS_BUS_BUFFER_LENGTH(&TxBufferInfoEth) > 0) {
        *eventHandlingNeeded = fmi3True;    // <6>
    }
}
----
<1> Creation of Ethernet frame payload.
<2> Definition of the destination address of the specified frame.
<3> Resetting of `fmi3LsBusUtilBufferInfo` variable instance.
<4> Creation of a Transmit operation and adding it to the specified `fmi3LsBusUtilBufferInfo` variable.
<5> Verify that free buffer space is available.
<6> Signal that `Event Mode` is needed.
...
fmi3Status fmi3GetClock(fmi3Instance instance, 
    ...
    for (size_t i = 0; i < nValueReferences; i++) { 
        if (valueReferences[i] == TX_CLOCK_REFERENCE) {
            if (!FMI3_LS_BUS_BUFFER_IS_EMPTY(&TxBufferInfoEth)) {   // <1>
                values[i] = fmi3ClockActive;                        // <2>
            } 
            else {
                values[i] = fmi3ClockInactive;                      // <3>              
            }
        }
    }
    ...
...
fmi3Status fmi3GetBinary(fmi3Instance instance, 
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == TX_DATA_REFERENCE) {
            values[i] = FMI3_LS_BUS_BUFFER_START(&TxBufferInfoEth);        // <4>
            valueSizes[i] = FMI3_LS_BUS_BUFFER_LENGTH(&TxBufferInfoEth);   // <5>
        }
    }
    ...
}
...
fmi3Status fmi3UpdateDiscreteStates(...) {
    ...
    /* Reset read/write positions */
    FMI3_LS_BUS_BUFFER_INFO_RESET(&TxBufferInfoEth);    // <6>
    ...
}
----
<1> Verify if Bus Operations exist for transfer.
<2> Activate specified `Tx_Clock`.
<3> Deactivate specified `Tx_Clock`.
<4> Get the start memory address of the buffer, by using `fmi3LsBusUtilBufferInfo` instance.
<5> Get the size of the buffer, by using `fmi3LsBusUtilBufferInfo` instance.
<6> Reset the transmission buffer so data is not transmitted again.
...
fmi3Status fmi3SetClock(fmi3Instance instance,
    ...
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == RX_CLOCK_REFERENCE) { // <1>
            /* Set an indicator that clock ticked and new Bus Operations arrived */
            RxClock = values[i]; // <2>
        }
    }
...
fmi3Status fmi3SetBinary(fmi3Instance instance,
    ...
    for (size_t i = 0; i < nValueReferences; i++) {
        if (valueReferences[i] == RX_DATA_REFERENCE) {   
            /* Reset read/write positions */
            FMI3_LS_BUS_BUFFER_INFO_RESET(&RxBufferInfoEth);                    // <3>
            /* Write buffer */
            FMI3_LS_BUS_BUFFER_WRITE(&RxBufferInfoEth, value[i], valueSize[i]); // <4>
        }
    }
    ...
}
...
<1> Check if `Rx_Clock` has ticked.
<2> Store the information for global access within other FMI interface functions.
<3> Reset buffer to remove obsolete reception data.
<4> Create an `fmi3LsBusUtilBufferInfo` instance based on received Bus Operations.
...
fmi3Status fmi3UpdateDiscreteStates(...)
{
    ...
    if (fmi3ClockActive == RxClock) {
        /* Processing of received Bus Operations */
        while (FMI3_LS_BUS_READ_NEXT_OPERATION(&RxBufferInfoEth, hdr))       // <1>
        {
            switch (hdr->type)                                               // <2>
            {
                case FMI3_LS_BUS_ETHERNET_OP_TRANSMIT:
                    fmi3LsBusEthernetOperationTransmit *receivedTransmitOp   // <3>
                        = (fmi3LsBusEthernetOperationTransmit*) hdr;
            ...
            }
        }
        ...
        /* Reset read/write positions */
        FMI3_LS_BUS_BUFFER_INFO_RESET(&RxBufferInfoEth);
    }
    ...
}
